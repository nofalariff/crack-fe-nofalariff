...
import { FileText, Pencil, Wallet } from "lucide-react"

import { PageHeader } from "@/components/shared/page-header"
import { CancelShipmentDialog } from "@/components/shipment/cancel-shipment-dialog"
...
export async function generateMetadata({
  params,
}: PageProps<"/kirim/[trackingNumber]">): Promise<Metadata> {
  const { trackingNumber } = await params
  return { title: `Kiriman ${trackingNumber}` }
}

export default async function DetailKirimanPage({
  params,
  searchParams,
}: PageProps<"/kirim/[trackingNumber]">) {
  ...
  const shipment = await getShipment(trackingNumber).catch((error) => {
    // Kiriman milik akun lain juga dibalas 404 oleh backend — tidak membocorkan
    // keberadaan datanya.
  ...
  const justUpdated = query.diperbarui === "1"

  const shipmentDetails: [string, string][] = [
    ...
    ["Dibuat pada", formatDateTimeWIB(shipment.createdAt)],
  ]
  ...
  return (
    <>
      <PageHeader
        ...
        action={
          <div className="flex flex-wrap gap-2">
            {needsPayment(shipment.status) && (
              <Button asChild>
                <Link href={`/kirim/${shipment.trackingNumber}/bayar`}>
                  <Wallet aria-hidden />
                  Bayar
                </Link>
              </Button>
            ...
            <Button asChild variant="outline">
              <Link href={`/kirim/${shipment.trackingNumber}/invoice`}>
                <FileText aria-hidden />
                Invoice
              </Link>
            </Button>
            {canCustomerEdit(shipment.status) && (
              <Button asChild variant="outline">
                <Link href={`/kirim/${shipment.trackingNumber}/ubah`}>
                  <Pencil aria-hidden />
                  Ubah
                </Link>
              </Button>
      ...
      {justUpdated && (
        <p
          role="status"
          className="border-success/30 bg-success-soft text-success-foreground rounded-lg border px-4 py-3 text-sm"
        >
          Data kiriman berhasil diperbarui.
        </p>
      )}
      ...
      <div className="grid gap-6 lg:grid-cols-[1fr_22rem]">
        <div className="min-w-0 space-y-6">
          ...
          <Card>
            <CardHeader>
              <CardTitle>Isi barang</CardTitle>
            </CardHeader>
          ...
          <Card>
            ...
            <CardContent className="space-y-3">
              ...
              {shipment.outstandingAmount > 0 && (
                <p className="bg-warning-soft text-warning-foreground rounded-md px-3 py-2 text-sm">
                  Belum dibayar: {formatRupiah(shipment.outstandingAmount)}
                </p>
              )}
            </CardContent>
          </Card>
